...
void setup() {
  ...
  */
  ...
  serverWEB.on("/TempDia", TemperaturaDia);
  serverWEB.on("/TempMes", TemperaturaMes);
  serverWEB.on("/TempAno", TemperaturaAno);
  //errores y root
...
void handleRoot() {
  ...
  botoncdia = "<p><a href='/TempDia'><button style='height:auto;width:auto;FONT-SIZE: 20pt'>Dia</button></a></p>";
  botoncmes = "<p><a href='/TempMes'><button style='height:auto;width:auto;FONT-SIZE: 20pt'>Mes</button></a></p>";
  botoncano = "<p><a href='/TempAno'><button style='height:auto;width:auto;FONT-SIZE: 20pt'>Ano</button></a></p>";
...
String pagina(String comandoTemperatura) {
  //TODO analizar todos los posibles estados y recargar variables boton.
  String estadoClima = llamarClima(comandoTemperatura);
  ...
  if (estadoClima.indexOf("</leyenda>")) {
    int posIni = estadoClima.indexOf("<leyenda>");
    int posFin = estadoClima.indexOf("</leyenda>");
    leyendas = estadoClima.substring(posIni + 13, posFin);
    leyendas = leyendas.substring(leyendas.lastIndexOf(","));
  }
  if ( estado.equals("Failed to open file for reading")) {
    ...
    graficaTemperatura =
      ...
      "});"
      "</script>"
      "<div id=\"container1\" style=\"width: 45%;margin-top: 20px; float:left; height: auto \"></div>";
 //      "<div id=\"container1\" style=\"width: 600px;margin-top: 20px; float:left; height: 400px \"></div>";
    ...
    graficaHumedad =
      ...
      "});"
      "</script>"
       "<div id=\"container2\" style=\"width: 45%;float:right;margin-top:20px; height: auto \"></div>";
  ...
  return "<html>"
         ...
         "<style> div1 {  float: left;  padding: 20px;  width: auto; height:auto;  background-color: #f1f1f1;} </style>"
...
void TemperaturaDia () {
  estado = llamarRiego("EstadoRiego");
  tratarRespuesta();
  paginaHtml = pagina(c_dia);
  serverWEB.send(200, "text/html", paginaHtml);
}
void TemperaturaMes() {
  estado = llamarRiego("EstadoRiego");
  tratarRespuesta();
  paginaHtml = pagina(c_mes);
  serverWEB.send(200, "text/html", paginaHtml);
}
void TemperaturaAno() {
  estado = llamarRiego("EstadoRiego");
  tratarRespuesta();
  paginaHtml = pagina(c_ano);
  serverWEB.send(200, "text/html", paginaHtml);
}
